from mobile import mobile
class android(mobile):

   
    def __init__(self, brand, sim, camera):

        #call given to parent class para con that is mobile 
        super().__init__(brand, sim)
        self.camera= camera


    def take_photo(self):
        print("Smile Please")

    def display_android(self):
        oppo.power()
        print(f"Sim {self.sim} inserted!")
        oppo.call()
        print(f"Android has {self.camera}")
        oppo.take_photo()
        print("-----------------------------------------")



#values passed to parent class para con through child class para con
oppo= android("OPPO", "Jio", "48mp")
oppo.display_android()

vivo= android("VIVO", "VI", "55mp")
vivo.display_android()

...
class customer(savingaccount, loan):
    ...
    def __init__(self, name, bal):
        ...
        #both parent classes(saving , laon) has para cons so confusion is para meter passing so one solution out of 4 solution is used that call it by classname so no confusion to of para passing to which class exactly directly defined classnam ein call
...
# call to para cons of bankaccount.py is done twice , once in customer.py and second time in laon and saving dirctly and indirectly
# so values to name , bal are setted twicely 
#so this is diamond problem 
# solution to this is in this folder hybrid_diamond_problem_solve
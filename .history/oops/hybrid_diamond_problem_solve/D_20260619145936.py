...
class D(B,C):
    ...
    def __init__(self,name, salary, age ):
        print("D cons")
        #using **kwargs we set value to all para of A, B, C in key value pair format so the confusion of para passing is avoided and twice setting value to parent class var is also avoided and calling the parent class para cons twicely is also avoided 
        #call to B, C para cons, and set value to var according to repective parameters
        super().__init__(
            name= name ,
            salary= salary,
            age= age )
